...
- **Auto-generated engine schemas** -- 107 JSON parameter schemas scraped from the [SerpApi playground](https://serpapi.com/playground), kept fresh by weekly CI.
...
Engine schemas are auto-generated by `build-engines.py`, which scrapes parameter data from the [SerpApi playground](https://serpapi.com/playground). A [GitHub Actions workflow](.github/workflows/update-engines.yml) runs weekly to keep them fresh.
...
├── test.sh                     # Plugin validation script
└── README.md
```
...
- **Stale engine params**: Run `python build-engines.py` to regenerate
